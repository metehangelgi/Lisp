    ...
    (make-handler
     ...
     (make-methods
      ...
      'HAVE-THING? (lambda (thing)
                     (memq thing things))
      'ADD-THING   (lambda (thing)(define (create-grade name location) 
...
(define (person self name birthplace)
  (let ((mobile-thing-part (mobile-thing self name birthplace))
	...
	(cond ((ask self 'HAVE-THING? thing)  ; already have it
      ...
      'HAS-A-THING-NAMED
      (lambda (name)
        (let ((things (ask me 'THINGS)))
          (filter (lambda (thing) (eq? name (ask thing 'NAME))) things))) 
      
      'HAS-A
      (lambda (type)
        (let ((things (ask me 'THINGS)))
          (filter (lambda (thing) (member type (ask thing 'TYPE))) things)))
...
(define (autonomous-person self name birthplace restlessness miserly)
  (let ((person-part (person self name birthplace)))
	...
	(ask our-clock 'ADD-CALLBACK
      ...
      'MOVE-AND-TAKE-STUFF
      (lambda ()
        ;; if time is 0 mod 10, then reduce health by 1
        (if (= (modulo (ask our-clock 'THE-TIME) 10) 0 )
            (ask self 'AGE))
...
(define (hack self name location cover-story action)
  (let ((mobile-part (mobile-thing self name location)))
    (make-handler
     ...
     (make-methods
      'COVER-STORY
      (lambda () cover-story)(create-teleporter 'teleporter (ask me 'location))
      'ACTION
      (lambda () action)
      'USE
