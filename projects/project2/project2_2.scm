...
;BANNER
;;; Exercise 0:
;some constants or procedures may be needed in my code
(define (rest lst)
  (cdr lst))
...
(define (segment-length seg)
  (let ((pt1 (start-seg seg))
        (pt2 (end-seg seg)))
        (sqrt
         (+ (square (- (x-of pt2) (x-of pt1)))
            (square (- (y-of pt2) (y-of pt1))))))
    )
...
(define (copy-to-end ptList)
  (append ptList (list (car ptList))))

;type : (list(point) -> list(segment))
(define (create-polygon ptlist)
  (define (create-polygon-seg pointList) ;subdefining to prevent main list
    (if (null? (rest pointList))
        '()  ;;if there is no more point pair / '() is another showing of nil
        (cons (make-seg (first pointList) (second pointList)) (create-polygon-seg (rest pointList)) ) ;if there is more than three points to process it. 
        )
  ) (create-polygon-seg (copy-to-end ptList))
)

; test:
test-points
(copy-to-end test-points)
...
(add-two-points (make-point 2 5) (make-point -1 3))
;should return (1 8)
...
(define (add-all-points lst)
  (if (null? (rest lst)) (first lst)
      (add-all-points (cons (add-two-points (first lst) (second lst)) (rest (rest lst))))
      )
)
...
; type : list(point) -> point
(define (centroid lst)
  (let ((sum-of-points (add-all-points lst)))
    (make-point
     (* (/ (x-of sum-of-points) (length lst)) 1.0)
     (* (/ (y-of sum-of-points) (length lst)) 1.0))
    )
)
...
(define (sum elementList)
  (if (null? elementList)
    0
    (+ (car elementList) (sum (cdr elementList)))
  )
)

;type : list(point)-> point
(define (centroid-alt lst)
  ;this gets list of x-coordinates or y-coordinates
  (define (coordinate points whichOne) 
    (if (null? points) '()
        (cons (whichOne (first points)) (coordinate (rest points) whichOne)))
    )
  ;then finds centroid by using x-List and y-list
  (make-point
   (* (/ (sum (coordinate lst x-of)) (length lst)) 1.0)
   (* (/ (sum (coordinate lst y-of)) (length lst)) 1.0)
   )
)
...
(display "because when we put cos we need a variable for it. cosine is a numerical function that's why it doesn't work. we can use symbol of cosine instead of its procedure")
(display "if there was high procedure inside of cos-square it would be work, which is like below procedure")
(newline)
...
;type : (number,number->number)->(F X F ->F)
...
(newline)
; test:
(((make-procedure-binop -) cos sin) 3.1415)    ;cos(pi)-sin(pi)
; yields -1
...
(display "deriv-squared procedure changes F to the ((F->F)->(F->F)). So if we write in wrong way as above our function looks to have type (number->number)->(F->F) because")
(display " while (deriv f) has type (F-F); deriv has type F")
(newline)
...
; return a number)?
(display "it should be 4 because thrice procedure sends f to compose to times. so by using repeated we have to do it n=4 times.")
(newline)
...
(display "(number ->(number->list(Curve))")
(newline)
...
; type (point X number)->(number->list(curve))
(define (diagonal-line pt len)
  (let ((x (x-of pt))
        (y (y-of pt)))
     (lambda (t) (make-point (+ x (* len (cos pi/4) t)) (+ y (* len (sin pi/4) t)))
                  )
    ))

;    3. What is the type of diagonal-line?

(display "point X number -> (number->list(point))")
(newline)
...
;type (number->list(point)) -> (number->point)
...
;type ((number->list(point)) X (number->list(point))) -> (number -> point)
...
;;these files were loaded at top of the project file already. 
(setup-windows)
...
;((draw-points-squeezed-to-window g2 200) unit-circle) 

(display "alternative-unit-circle creates more point then connect them
that's why this method is more precise??")
(newline)
...
; this pocedure shows same thing with show-connected-gosper without connected them(with points). Also can do procedure with any kind of curves with repeated number of times.
;By this way I can get much more evaluated sketches. 
...
;(show-points-gosper g3 1000 3000 arc)
...
;I could do put my code into your-answer-here part, Howver by this way my code is more eligible and neat. 
...
;((draw-points-on g3 1000) (param-gosper 8 (lambda (level) pi/4)))
...
;((draw-points-squeezed-to-window g1 500) (param-gosper 8 (lambda (level) (/ pi (+ level 2)))))
;((draw-points-squeezed-to-window g2 2000) (param-gosper 8 (lambda (level) (/ pi (expt 1.3 level)))))
;if we don't give at least 1000 points for second we couldn't get logical picture
...
; I couldn't do this method by using param-gosper becuase I need to use origin curve number of times.
;but our param-gosper does not get initial curve(just uses unit-line). That's why I am doing part by part in recursively. 
(define (kochize level)
  (if (= level 0)
      unit-line
      (let ((c (kochize (- level 1))))
        (put-in-standard-position
         (connect-ends (connect-ends c
                                     ((rotate-around-origin (/ pi 3)) c))
                       (connect-ends ((rotate-around-origin (- (/ pi 3))) c)
                                     c))))))



((draw-connected g2 200) (kochize 4))
...
;((draw-connected-squeezed-to-window g3 200) (kochize 4))
;((draw-connected-squeezed-to-window g2 200) (kochize 3))
((draw-connected-squeezed-to-window g1 1000) (kochize 2))
...
;I have tried to use list to mapping along the list. However while doing om that list, my code cannot finish procedure. That's why I used just 1 value to sketch something. It does not like specific object.
; But at I could use this  method by this way.
((draw-connected-squeezed-to-window g3 200)
 ((lambda (level)
    ((deriv-t 1) (gosper-curve level))) 6))
